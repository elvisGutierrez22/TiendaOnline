<?php
const BASE_URL = "https://tiendare-448695688435.us-central1.run.app/";
const HOST = "34.71.146.244";
const USER = "root";
const PASS = "administracion2024";
const DB = "tienda_online";
const CHARSET = "charset=utf8";
const TITLE = "TIENDA ONLINE";
const MONEDA = "USD";
const CLIENT_ID = "AZOwC5luK4kTRZxje_X9XgLSth4eGfJ884ix_0ejpaYjS0iW8sqOMkDv51JVg9oLFvcgQucQkg6_P1m_";
const USER_SMTP = "francis1522v@gmail.com";
const PASS_SMTP = "gxjkrfyqdvgbeomg";
const PUERTO_SMTP = "465";
const HOST_SMTP = "smtp.gmail.com";
?>
